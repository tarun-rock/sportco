...
@section("meta_description", $sport->desc ?? "")
                                                ...
                                                @else
                                                    <a href="{{ url("article") }}/{{$post->slug}}">
                                                        ...
                                                        <div class="thumb-container">
                                                            ...
                                                            <img data-src="@if(empty($post->sportsgram_url)) {{$post->media_url}}@else{{$post->sportsgram_url}}@endif"
                                                                 src="img/empty.png" class="entry__img lazyload"
                                                                 alt=""/>
                                                            {{--<img data-src="{{$post->sportsgram_url ?? "d"}}" src="img/empty.png" class="entry__img lazyload" alt="" />--}}
                                                        </div>
                                                    </a>
                                                    <a href="{{ url("sport") }}/{{$post->sports_name }}"
                            ...
                            <ul class="pagination">
                                <li class="pagination__page {{ ($posts->currentPage() == 1) ? ' disabled' : '' }}">
                                    <a class="pagination__page pagination__icon pagination__page--next"
                                       href="{{ $posts->url(1) }}"><i class="ui-arrow-left"></i></a>
                                </li>
                                @for ($i = 1; $i <= $posts->lastPage(); $i++)
                                    ...
                                    @if ($from < $i && $i < $to)
                                        <li class="pagination__page {{ ($posts->currentPage() == $i) ? ' active' : '' }}">
                                            <a class="pagination__page {{ ($posts->currentPage() == $i) ? ' pagination__page--current' : '' }}"
                                ...
                                <li class="pagination__page pagination__icon pagination__page--next {{ ($posts->currentPage() == $posts->lastPage()) ? ' disabled' : '' }}">
                                    <a class="pagination__page pagination__icon pagination__page--next"
                                       href="{{ $posts->url($posts->lastPage()) }}"><i class="ui-arrow-right"></i></a>
                                </li>
                            </ul>
                        </nav>
                    @endif
